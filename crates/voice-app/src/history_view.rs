...
use gpui_component::{button::*, h_flex, v_flex, ActiveTheme, Sizable};
            ...
            v_flex()
                ...
                .child(
                    h_flex()
                        .items_center()
                        .gap_3()
                        .child(div().text_xl().font_weight(FontWeight::BOLD).child("History"))
                        .child(div().flex_1())
                ...
                .child(if self.files.is_empty() {
                    div().text_sm().text_color(theme.muted_foreground).child("No calls yet. Transcripts are saved automatically when you hang up.").into_any_element()
                } else {
                    v_flex()
                        .gap_1()
                        .children(self.files.iter().map(|(name, path)| {
                            let path = path.clone();
                            Button::new(SharedString::from(name.clone())).ghost().label(name.replace('_', " ")).on_click(move |_, _, _| {
